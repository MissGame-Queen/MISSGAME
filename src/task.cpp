...
void SoundPlayer(uint16_t playerNumber)
{
    ...
    if (playerNumber == 0)
    {
        if (!isInit)
        {
            ...
            {
                state[i] = myDFPlayer[i].begin(i == 0 ? Serial2 : Serial1, /*isACK = */ true, /*doReset = */ true);
                if (state[i])
                {
    ...
    */
     ...
     */
        case 30 ... 89:
        {
            ...
            bool isAutoPam = true;
            if (
                docWeaponLight.containsKey("_LIGHT_0") &&
                docWeaponLight.containsKey("_LIGHT_1") &&
                docWeaponLight.containsKey("_LIGHT_2") &&
                docWeaponLight.containsKey("_LIGHT_3"))
            {
                if (docWeaponLight[_LIGHT_0].as<uint16_t>() != 0 ||
                    docWeaponLight[_LIGHT_1].as<uint16_t>() != 0 ||
                    docWeaponLight[_LIGHT_2].as<uint16_t>() != 0 ||
                    docWeaponLight[_LIGHT_3].as<uint16_t>() != 0)
                    isAutoPam = false;
            }
            if (isAutoPam)
            {
            ...
            case 99:
            {
                ...
                {
                    color = setRainbowRGB((map(millis() % 1000, 0, 1000, 0, 1536) + map(i, 0, strip.numPixels(), 0, 1536)) % 1536);
                    color = setBrightnessRGB(color, num < 30 ? map(num, 0, 31, 0, 125) : map(num, 31, 61, 125, 0));
                    strip.setPixelColor(i, color);
                }
            }
            break;
 ...
 *
 */
void taskLINE_POST(String message)
{
