...
void runType()
{
    ...
    if (lastValue != intType)
    {
        ...
        case 0:
        {
            ...
            intLingth > 0 ? analogWrite(pinOutput[0], analogMaxValue) : digitalWrite(pinOutput[0], !onSide);
    ...
    else
    {
        ...
        {
            uint32_t valueTimer = millis() - timer;
            const uint16_t caseTimer_0 = 200;
            if (valueTimer > 4000 && valueTimer <= 4000 + 20000)
            {
                digitalWrite(pinOutput[0], !onSide);
            }
            else
            {
                valueTimer = valueTimer % caseTimer_0;
                if (valueTimer < caseTimer_0 / 2)
                    analogWrite(pinOutput[0], map(valueTimer, 0, caseTimer_0 / 2, 0, analogMaxValue));
                else
                    analogWrite(pinOutput[0], map(valueTimer, caseTimer_0 / 2, caseTimer_0, analogMaxValue, 0));
            }
        }
        break;
            // D:
